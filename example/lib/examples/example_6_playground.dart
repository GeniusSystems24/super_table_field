...
class _PlaygroundExampleState extends State<PlaygroundExample> {
    ...
    for (var i = 0; i < count; i++) {
      ...
      out.add(
        SuperRow.map({
          'sku': 'SKU-${1100 + n}',
          'name':
              '${_genNames[n % _genNames.length]} ${(n ~/ _genNames.length) + 1}',
          ...
          'updated':
              '2024-${(11 + (n % 2)).toString().padLeft(2, '0')}-${((n % 27) + 1).toString().padLeft(2, '0')}',
  ...
  List<SuperRow<_Row>> _initialRows() => [
    for (final p in _products) SuperRow.map(_seedRow(p)),
  ];
  ...
  void initState() {
    ...
    _c = SuperTableController<_Row>(
      ...
      onNotify: (kind, msg) => _notify(
        kind == SuperNotifyKind.error
                ...
                Expanded(
                  child: ListenableBuilder(
                    ...
                    builder: (ctx, _) => SuperTable<_Row>(
                      ...
                      onAddColumn: _c.mode == SuperTableMode.editable
                          ? _addColumn
                          : null,
      ...
      builder: (ctx, _) => Wrap(
        ...
        children: [
          ...
          _seg('Select', [
            ...
            (
              'Cells',
              _c.selectionMode == SuperSelectionMode.multiCells,
              () => _c.setSelectionMode(SuperSelectionMode.multiCells),
            ),
            (
              'Row',
              _c.selectionMode == SuperSelectionMode.singleRow,
              () => _c.setSelectionMode(SuperSelectionMode.singleRow),
            ),
            (
              'Rows',
              _c.selectionMode == SuperSelectionMode.multiRows,
          ...
          _seg('Paging', [
            ...
            (
              'Pages',
              _c.pagination == SuperPagination.pages,
              () => _setPaging(SuperPagination.pages),
            ),
            (
              'Load+',
              _c.pagination == SuperPagination.loadMore,
              () => _setPaging(SuperPagination.loadMore),
            ),
            (
              'Infinite',
              _c.pagination == SuperPagination.infinite,
              () => _setPaging(SuperPagination.infinite),
            ),
          ]),
          _chip(
            t,
